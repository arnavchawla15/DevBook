...
import { Sparkles, History, TextCursorInput, Box, BookOpen, Network } from 'lucide-react';
...
function App() {
  const [mode, setMode] = useState<'line' | 'block' | 'flowchart'>('flowchart');
          ...
          <header className="header-container glass-panel">
            ...
            <div className="mode-selector">
              <button
                ...
                onClick={() => {
                  setMode('line');
                  if (codeContent.trim()) handleRunCode('line');
                }}
              >
              ...
              <button
                ...
                onClick={() => {
                  setMode('block');
                  if (codeContent.trim()) handleRunCode('block');
                }}
              >
                <Box size={16} /> Explain Block
              </button>
              <button
                className={`mode-btn ${mode === 'flowchart' ? 'active' : ''}`}
                onClick={() => {
                  setMode('flowchart');
                  if (codeContent.trim()) handleRunCode('flowchart');
                }}
              >
